...
import { Datacenter, Disk } from '@/services/asset';
...
type DiskModalProps = {
  ...
  initialData?: Partial<Disk>;
  onSubmit: (data: Partial<Disk>) => Promise<void>;
  onDelete?: (id: number) => void;
};

export default function DiskModal({
  isOpen,
  onClose,
  mode,
  datacenters,
  ...
  const [formData, setFormData] = useState<Partial<Disk>>(initialData || {});
  ...
  const handleChange = <K extends keyof Disk>(field: K, value: Disk[K]) => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[800px] m-4">
      <div className="w-full p-4 bg-white rounded-3xl dark:bg-gray-900 lg:p-11">
        <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
          {mode === 'edit' ? 'Edit Disk Array' : 'Add Disk Array'}
        </h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          {mode === 'edit'
            ? 'Update disk information.'
            : 'Enter disk details to add a new disk array.'}
        </p>
